...
| **CF-10** | **Third-party procedural guide** — `docs/THIRD-PARTY-GUIDE.md` **stub created** + a **sync guard** (`scripts/check-guide-pins.mjs` / `npm run check.guide`, wired into `ci.yml` + `release.yml`) that **fails CI if the guide's pinned toolchain drifts from `package.json`**. **Carry-forward:** flesh out the 9 step sections (still `_(expand)_`), and on every release bump the guide's `toolchain-pins` block to match `package.json` (the guard enforces it; see the guide's "Release checklist"). README links to it. | this repo | MED |
